...
EOF
        ...
        rsync --no-perms --no-group --no-owner -rl "$MATOMO_BASE_DIR"/plugins/ "$MATOMO_VOLUME_DIR"/plugins/
        # Update default js files
        info "Updating default js scripts in persisted data"
        rsync --no-perms --no-group --no-owner -rl "$MATOMO_BASE_DIR"/js/ "$MATOMO_VOLUME_DIR"/js/
